...
dotcol  = subwrd(args,4)
dotsize = subwrd(args,5)
bigdot  = subwrd(args,6)
marktype = subwrd(args,7)
lincol  = subwrd(args,8)
colname = subwrd(args,9)
* say filenum" "latname" "lonname" "colour" "dotsize" "bigdot" "marktype
...
if (tnow > 1010000)
  ...
  "q w2xy "lonnow" "latnow
  rec = sublin(result,1)
  xnow = subwrd(rec,3)
  ynow = subwrd(rec,6)
#  say "q w2xy "lonnow" "latnow
  if (latnow >= latminv & latnow <= latmaxv & lonnow >= lonminv & lonnow <= lonmaxv)
    "set line 2"  ;*dotcol
    "draw mark "marktype" "xnow" "ynow" "bigdot
    biglat = latnow
    biglon = lonnow
  endif
else

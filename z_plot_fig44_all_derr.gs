...
mul.1 = 3 ; tiny.1 = 1e-5 ; labelx.1 = _retmid.1-0.1" 10.45 a) Spd (10`a-"mul.1"`nms`a-1`n)"
mul.2 = 1 ; tiny.2 = 1e-4 ; labelx.2 = _retmid.2" 10.45 b) Dir (10`a-"mul.2"`ndeg)"
mul.3 = 3 ; tiny.3 = 1e-4 ; labelx.3 = _retmid.3" 10.45 c) U (10`a-"mul.3"`nms`a-1`n)"
mul.4 = 3 ; tiny.4 = 1e-4 ; labelx.4 = _retmid.4" 10.45 d) V (10`a-"mul.4"`nms`a-1`n)"
  ...
  b = 1 ; while (b < 4)
    ...
    c = 1 ; while (c <= xpic)
      ...
      d = 1 ; while (d <= levs)
        "set z "levs
        "set gxout stat"
        "d "var.c ; line = sublin(result,8) ; minlev  = subwrd(line,4) + tiny.c
        if (minlev < glomin.a.b.c) ; glomin.a.b.c = minlev ; endif
        d = d + 1
      endwhile
      glomin.a.b.c = glomin.a.b.c - tiny.c
  ...
  b = 1 ; while (b < 3)
    ...
    c = 1 ; while (c <= xpic)
      ...
      d = 1 ; while (d <= ypic)
        ...
        "set lat -0.5 6.5"
        "set lon -0.5 6.5"
        "set xlabs | 0.5 | | | | 3.5 | | | | 15.5 | | | | Inf |"
        "set ylabs | 0.5 | | 1.5 | | 3.5 | | 7.5 | | 15.5 | | 31.5 | | Inf |"
        "set ccols    49    45    0"
        "set clevs      0.25  0.75"
        if (minlev < masklev)
...
#        "set grads off" ; "d "var.c
        "set grads off" ; "d pow(10,"mul.c")*abs("var.c"-"glomin.a.b.c")"
        d = d + 1
      endwhile
      c = c + 1
    endwhile
...
#    "draw string 4.65 10.80 Analysis - Buoy Standard Deviation"
    ...
    "draw string 4.70  8.35 SAR Dir Error V-Component"
    "set string 1 bc 6 90"
    "draw string 0.20  9.5 SAR Dir Error"
    "draw string 0.50  9.5 U-Component"
    "set string 1 bc 3 0"

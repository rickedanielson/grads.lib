...
*
*        grads -bpc "plot.growth.rates.energy.eddy.conversion 756100"
*
...
"run gui_print plot."tag"."budget".conversion.volavg"
"quit"
...
mrk.1 = 4                                                          ;# reset marker types to conform
mrk.2 = 5                                                          ;# to those of the eke budget
mrk.3 = 2
mrk.4 = 3
mrk.5 = 7
...
while (a < 5)                                                      ;# rates, smoothed in time
  ...
  while (b < 5)
    var = var.b""dom.a
    if (b = 4)                                                     ;# include other terms in residual
      var2 = "gvfc"dom.a
      say "define "var"avg = tloop((const(("var"(t-1)+"var2"(t-1))*"side",0,-u) + ("var"+"var2")*"cent" + const(("var"(t+1)+"var2"(t+1))*"side",0,-u)) / (2*"side"+"cent"))"
          "define "var"avg = tloop((const(("var"(t-1)+"var2"(t-1))*"side",0,-u) + ("var"+"var2")*"cent" + const(("var"(t+1)+"var2"(t+1))*"side",0,-u)) / (2*"side"+"cent"))"
    else
      say "define "var"avg = tloop((const("var"(t-1)*"side",0,-u) + "var"*"cent" + const("var"(t+1)*"side",0,-u)) / (2*"side"+"cent"))"
          "define "var"avg = tloop((const("var"(t-1)*"side",0,-u) + "var"*"cent" + const("var"(t+1)*"side",0,-u)) / (2*"side"+"cent"))"
    endif
    ...
    if (b = 4)
      say "d ave("var"+"var2", t="tima", t="timb")"
          "d ave("var"+"var2", t="tima", t="timb")"
    else
...
"set string 1 c"
"draw string 4.25 10.8 "tag" Eddy Energy Conversion (Time Smoothing "side"-"cent"-"side")"
"draw string 4.25 0.15 Note: residual term (cres) includes gvfc term"

...
*
*     grads -bpc "plot.budget.temperature.verint.gb compare_1986 45 -65"
*
...
title.0 = "Temperature ("conin.0" K)"
title.1 = "Q-L 12-h Tendency ("conin.1" K/s)"
title.2 = "Q-L Hor Flux Conv ("conin.2" K/s)"
...
while (a < 6)
  ...
  while (b <= timb)
    ...
    if (a = 2)
      "set clab off"
      "run disp_shaded_nozero 9.8*"var.a"/(sfcpres-100) "conin.a
      if (c = 6)
        "run disp_vector utempvint vtempvint 1e8"
      else
        "run disp_vector_nolab utempvint vtempvint 1e8"
      endif
    else
      if (a = 0)
        "set clab on"
        "run disp_unshaded_nozero 9.8*"var.a"/(sfcpres-100) "conin.a" 200"
      else
        "run disp_shaded_nozero 9.8*"var.a"/(sfcpres-100) "conin.a
      endif
    endif

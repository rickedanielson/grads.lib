...
defa = "ellip = GRAV * (cdiff(cdiff(hgt, x) / delx, x) / delx +"
defb =                 "cdiff(cdiff(hgt, y) / dely, y) / dely) +"
defc =         "pow(coriol,2.) / 2."
  say  "  "defa" "defb" "defc ; say ""
  "define "defa" "defb" "defc

#defa = "ellip2 = GRAV * ((hgt(x+1) - 2. * hgt + hgt(x-1)) / pow(delx,2.) +"
#defb =                  "(hgt(y+1) - 2. * hgt + hgt(y-1)) / pow(dely,2.)) + pow(coriol,2.) / 2."
#  say  "  "defa" "defb ; say ""
#  "define "defa" "defb
